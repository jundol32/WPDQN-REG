...
Target update:
- Call agent.hard_target_update() from main as before.
- If config['tau'] < 1.0 → performs SOFT update with tau (Polyak).
- If config['tau'] >= 1.0 or None → performs HARD update (tau=1.0).
"""
...
class WPDQNAgent(flax.struct.PyTreeNode):
    ...
    def update(agent, batch: Batch):
        ...
        def loss_fn(q_params):
            ...
            avi_reward_scale = agent.config.get('avi_reward_scale', 1.0)  # α (TD only)
            # EXTERNAL (optimization-only) symmetric loss scale
            loss_scale_s = agent.config.get('loss_scale_s', 10.0)         # s (small & fixed)
        ...
        # NOTE: target update는 main에서 agent.hard_target_update()를 호출해 주세요.


        new_target_q_network = target_update(new_q_network, agent.target_q_network, agent.config['tau'])

        return agent.replace(q_network=new_q_network, target_q_network=new_target_q_network), info
...
def create_learner(
        ...
        discount: float = 0.99,
        target_update_freq: int = 50,   # main에서 이 주기로 hard_target_update() 호출
        loss_p: float = 4.0,
        tau: float = 0.005,             # << NEW: tau<1.0이면 soft update, >=1.0이면 hard
        **kwargs):
    ...
    config = flax.core.FrozenDict(dict(
        discount=discount,
        target_update_freq=target_update_freq,  # main 쪽에서 사용
        loss_p=loss_p,
        tau=tau,                                # << NEW: soft/hard 선택자
...
def get_default_config():
    ...
    return ml_collections.ConfigDict({
        ...
        'hidden_dims': (256, 256),
        'discount': 0.99,
        'target_update_freq': 80,  # main이 이 주기로 hard_target_update() 호출
        'loss_p': 10.0,
        'tau': 0.005,              # << NEW: 0.005로 soft 업데이트(Polyak) 권장
