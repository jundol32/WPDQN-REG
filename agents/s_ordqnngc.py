...
class ORDQNNGCAgent(flax.struct.PyTreeNode):
    ...
    def hard_target_update(agent):
        """
        ### 변경된 부분 ###
        이전 hard_target_update 로직을 제거하고, 단순히 agent를 그대로 반환하도록 하여
        메인 학습 코드의 수정을 피합니다. Soft update는 매 스텝 update() 함수에서 이루어집니다.
        """
        return agent
...
def get_default_config():
    ...
    return ml_collections.ConfigDict({
        ...
        'hidden_dims': (256, 256),
        'discount': 0.99,
        'tau': 0.005, # target_update_freq 대신 tau로 변경 ### 변경된 부분 ###
        # 'target_update_freq' : 5, # ### 변경된 부분 ###
        'loss_p': 4.0,
    })